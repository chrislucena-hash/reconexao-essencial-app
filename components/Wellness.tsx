...
const Wellness: React.FC<WellnessProps> = ({ setView }) => {
  ...
  const playVoicePassage = async (base64: string, textToSpeak: string) => {
    ...
    try {
      await playVoicePassageService(audioData, textToSpeak);
    } catch (e) {
      console.warn("playVoicePassageService error:", e);
    } finally {
      setIsPlayingAudio(false);
    }
  };
      ...
      <div className="px-4 space-y-8">
          ...
          <div className="glass-mystic p-10 sm:p-12 rounded-[4rem] text-center space-y-10 flex flex-col items-center justify-center min-h-[580px] animate-in zoom-in border border-magic-gold/30 relative overflow-hidden shadow-[0_0_120px_rgba(212,175,55,0.15)] bg-gradient-to-b from-white/[0.02] to-transparent">
                ...
                <div className="animate-breath relative">
                  ...
                  {countdown !== null && (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-5xl font-black text-[#18245C] drop-shadow-[0_0_15px_rgba(24,36,92,0.3)]">{countdown}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {ritualStep === 'completed' ? (
              <button 
                ...
                className="relative z-10 px-10 py-5 bg-[#18245C] text-white rounded-full font-black text-[10px] uppercase tracking-[0.3em] hover:scale-105 active:scale-95 shadow-2xl transition-all border border-magic-gold/20"
              >
                Concluir Prática
              </button>
            ) : (
              <button 
                onClick={stopRitual} 
                className="relative z-10 px-8 py-3.5 bg-[#18245C]/10 text-[#18245C] hover:bg-[#18245C]/20 rounded-full font-black text-[9px] uppercase tracking-[0.3em] transition-all border border-[#18245C]/10"
              >
                Encerrar Prática
              </button>
            )}
        ...
        {setView && (
          <NextStepGuide 
            ...
            nextStepLabel="Ver Relatório de Evolução"
            onNavigate={() => setView(AppView.EVOLUTION)}
            message="Sua energia vital foi harmonizada. Acesse seu Relatório de Evolução para acompanhar seus registros e despertares."
          />
        )}
      </div>
    </div>

...
const Guidance: React.FC<GuidanceProps> = ({ setView }) => {
  ...
  const [fastingFinishedNotification, setFastingFinishedNotification] = useState<string | null>(null);

  useEffect(() => {
    if (!isFasting || !fastingStartTime) {
      setFastingTimeLeft(0);
      return;
    }
    ...
    const updateTimer = () => {
      ...
      if (diffMs <= 0) {
        ...
        if (typeof window !== 'undefined' && 'Notification' in window && Notification.permission === 'granted') {
          try {
            new Notification('Pausa Sagrada Concluída! ✨', {
              body: message,
              icon: '/favicon.ico'
            });
          } catch (e) {
  ...
  }, [isFasting, fastingStartTime, fastingWindow]);
  ...
  const formatFastingTimeLeft = (totalSeconds: number) => {
    ...
    if (insight?.dailyExercise) {
      ...
      if (!audioCacheRef.current[text]) {
        generateSpeech(text).then((audio) => {
          if (audio) audioCacheRef.current[text] = audio;
        }).catch(() => {});
      }
    }
  }, [insight]);
  ...
  const fallbackSpeech = (textToSpeak: string) => {
    ...
    if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
      ...
      utterance.rate = 1.05; // Velocidade fluida, natural e dinâmica
  ...
  const playGuidance = async (text: string, customInstruction?: string) => {
    ...
    if (cached) {
      await playVoicePassageService(cached, text);
      setIsPlaying(false);
      return;
    }

    // Se o áudio ainda não estiver em cache, solicitar via servidor Gemini TTS
    ...
    try {
      ...
      audioBase64 = await generateSpeech(text, prompt);
      if (audioBase64) {
        audioCacheRef.current[text] = audioBase64;
      }
    } catch (e) {
      console.warn("Speech generation error, using synthesis fallback:", e);
    } finally {
      setLoadingAudioText(null);
    }

    await playVoicePassageService(audioBase64, text);
    setIsPlaying(false);
  };
  ...
  return (
    <div className="p-4 pt-safe pb-safe-nav max-w-2xl mx-auto space-y-8 animate-in fade-in">
       <header className="space-y-1 text-center">
          <h2 className="text-4xl font-serif text-[#18245C] italic">Bússola da Alma</h2>
          <p className="text-aura-gold text-[10px] font-black uppercase tracking-widest">Portal do Guia</p>
       </header>

       <div className="flex glass-mystic p-1 rounded-2xl gap-1 mx-2 shadow-xl overflow-x-auto no-scrollbar">
          <button onClick={() => setActiveSubTab('jornada')} className={`flex-1 min-w-[80px] py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'jornada' ? 'bg-[#18245C] text-white shadow-md' : 'text-[#4A506B]'}`}>Jornada</button>
          <button onClick={() => setActiveSubTab('autocura')} className={`flex-1 min-w-[80px] py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'autocura' ? 'bg-[#18245C] text-white shadow-md' : 'text-[#4A506B]'}`}>Autocuidado</button>
          <button onClick={() => setActiveSubTab('saude-intestinal')} className={`flex-1 min-w-[80px] py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeSubTab === 'saude-intestinal' ? 'bg-[#18245C] text-white shadow-md' : 'text-[#4A506B]'}`}>Saúde Intestinal</button>
       </div>
      ...
      {activeSubTab === 'jornada' && content && (
        <div className="space-y-8 animate-in fade-in">
          ...
          <section className="relative p-8 sm:p-10 glass-mystic rounded-[4rem] border border-aura-violet/30 bg-aura-violet/5 text-center space-y-6 shadow-2xl overflow-hidden group">
            ...
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 p-3 bg-white border border-aura-violet/30 rounded-full text-aura-violet shadow-md">
      ...
      {activeSubTab === 'autocura' && content && (
        <div className="space-y-8 animate-in slide-up">
           <div className="px-4 text-center space-y-2">
              <h3 className="text-2xl font-serif text-[#18245C] italic">Tríade Sagrada</h3>
             ...
             {content.menu.map((recipe, index) => (
               <section key={index} className="relative glass-mystic rounded-[3.5rem] border border-white/5 overflow-hidden group transition-all shadow-2xl">
                  ...
                  <div className="p-8 space-y-6">
                     <div className="flex justify-between items-start">
                       <div className="space-y-1">
                         <div className="flex items-center gap-2 mb-1">
                           ...
                           <div className="flex items-center gap-1 bg-[#18245C]/5 px-2 py-0.5 rounded-md border border-[#18245C]/10">
                       ...
                       <div className="flex gap-2">
                         <button 
                           ...
                           aria-label="Outras opções de receitas"
                         >
                           <RotateCcw size={18} />
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-[#18245C]/10">
                       <div className="space-y-3">
                         ...
                         <ul className="space-y-2">
                           {recipe.ingredients.map((ing, i) => (
                             <li key={i} className="text-xs text-[#4A506B] flex items-center gap-3 italic">
                       ...
                       <div className="space-y-3">
                         ...
                         {recipe.instructions.map((inst, i) => (
                           <p key={i} className="text-xs text-[#4A506B] leading-relaxed pl-4 border-l border-[#18245C]/10 italic">
                             {inst}
                           </p>
                         ))}
                       </div>
      ...
      {setView && activeSubTab === 'jornada' && (
        <NextStepGuide 
          currentStepName="Portal do Guia - Jornada"
          stepNumber={4}
          totalSteps={7}
          nextStepName="Aba de Autocuidado"
          nextStepLabel="Ir para Autocuidado"
          onNavigate={() => {
            setActiveSubTab('autocura');
            window.scrollTo({ top: 0, behavior: 'smooth' });
          }}
          message="Siga para a aba de Autocuidado para descobrir as alquimias e a nutrição consciente para o seu templo."
        />
      )}

      {setView && activeSubTab === 'autocura' && (
        <NextStepGuide 
          currentStepName="Portal do Guia - Autocuidado"
          stepNumber={4}
          totalSteps={7}
          nextStepName="Aba de Saúde Intestinal"
          nextStepLabel="Ir para Saúde Intestinal"
          onNavigate={() => {
      ...
      {setView && activeSubTab === 'saude-intestinal' && (
        <NextStepGuide 
      ...
      {showOptionsModal && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-xl flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-300">
           <div className="bg-white border border-[#18245C]/20 w-full max-w-lg rounded-[3rem] overflow-hidden max-h-[85vh] flex flex-col shadow-2xl">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
                 {recipeOptions.map((opt, i) => (
                    <button 
                      ...
                      className="w-full p-6 glass-mystic border border-[#18245C]/10 rounded-[2rem] text-left hover:border-aura-gold/50 transition-all group flex items-center justify-between gap-4 shadow-sm"
                    >
                       <div className="space-y-1">
                          <div className="flex items-center gap-2">
                          ...
                          <p className="text-[10px] text-[#4A506B] line-clamp-1 italic">
                            {opt.ingredients.slice(0, 3).join(', ')}...
                          </p>
                       </div>
                       <ChevronRight className="text-[#4A506B] group-hover:text-aura-gold transition-transform group-hover:translate-x-1" size={20} />
                    </button>
                 ))}
              </div>
              <footer className="p-6 text-center border-t border-[#18245C]/10 bg-[#18245C]/5">
                 <p className="text-[10px] text-[#4A506B] italic">Cada opção utiliza uma base de ingredientes distinta para sua evolução.</p>
              </footer>
           </div>
        </div>
      )}

      {/* Pausa Sagrada Completion Notification Modal */}
      {fastingFinishedNotification && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in">
          <div className="p-8 max-w-md w-full rounded-[3rem] border border-[#18245C]/20 bg-white text-center space-y-6 shadow-2xl">
            <div className="p-4 bg-aura-violet/20 rounded-full w-16 h-16 mx-auto flex items-center justify-center text-aura-violet border border-aura-violet/30 animate-pulse">
              <Bell size={32} />
            </div>
            <div className="space-y-2">
            ...
            <button
              onClick={() => setFastingFinishedNotification(null)}
              className="w-full py-4 rounded-2xl bg-[#18245C] text-white font-black text-xs uppercase tracking-widest hover:bg-[#203078] active:scale-95 transition-all shadow-lg"
            >
              Agradecer & Continuar
            </button>
          </div>

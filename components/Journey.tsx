...
import { JourneyDay, JourneyProgress, AppView } from '../types';
...
const Journey: React.FC<JourneyProps> = ({ progress, onUpdateProgress, onResetJourney, setView }) => {
  ...
  return (
    <div className="p-4 pt-safe pb-safe-nav max-w-2xl mx-auto space-y-8 animate-in fade-in">
      ...
      {setView && (
        <NextStepGuide 
          currentStepName="Portal da Senda"
          stepNumber={2}
          totalSteps={7}
          nextStepName="Portal do Diário (Livro de Espelhos)"
          nextStepLabel="Abrir Diário de Espelhos"
          onNavigate={() => setView(AppView.TRACKER)}
          message="Após vivenciar a reflexão e ativar seu portal diário, registre sua jornada no Livro de Espelhos."
        />
      )}
